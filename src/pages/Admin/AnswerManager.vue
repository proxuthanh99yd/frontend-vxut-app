<template>
  <b-container>
    ...
    ></el-page-header>
    <b-card>
      <h4 v-html="answerData.question.question"></h4>
      <b-img
        v-if="answerData.question.photo"
        :src="baseImg + answerData.question.photo"
        fluid-grow
        alt="Fluid-grow image"
      ></b-img>
      <audio v-if="answerData.question.audio" controls>
        <source :src="baseAudio + answerData.question.audio" type="audio/ogg" />
      </audio>
      <b-button class="mt-2 mb-2" block @click="create" variant="success"
        ><b-icon icon="plus-circle" aria-hidden="true"></b-icon
      ></b-button>
      <!-- User Interface controls -->
      <b-list-group v-for="(value, key) in answerData.answer" :key="key"
...
export default {
  ...
  created() {
    this.getAnswerData(this.$route.params.question_id);
  },
  computed: {
    ...mapGetters({
  ...
  methods: {
    ...mapActions("answer", ["getAnswerData"]),
    ...mapActions("answer", ["storeAnswerRequest"]),
    ...mapActions("answer", ["updateAnswerRequest"]),
    ...mapActions("answer", ["destroyAnswerRequest"]),
    create() {
      this.infoModal.title = "Create";
      this.formData.question_id = "";
    ...
    edit(value, button) {
      ...
      this.formData.question_id = value.question_id;
    ...
    del(row) {
      this.$bvModal
        ...
        .then((value) => {
          if (value === true) {
            this.destroyAnswerRequest(row.id).then(() => {
              this.getCustomers();
            });
          }
        })
    ...
    handleSubmit() {
      ...
      if (this.formData.isEdit == true) {
        this.updateAnswerRequest(this.formData).then(() => {
          this.getCustomers();
        });
      } else {
        this.formData.question_id = this.$route.params.question_id;
        this.storeAnswerRequest(this.formData).then(() => {
          this.getCustomers();
        });
      }
      this.$nextTick(() => {
