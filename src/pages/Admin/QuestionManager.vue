    ...
    <b-card>
      ...
      >
        <template #cell(question)="data">
          <router-link
            custom
            :to="{ name: 'Answer', params: { question_id: data.item.id } }"
      ...
      >
        ...
        >
          ...
          <b-form-group label="Audio File:" label-cols-sm="2"
            ...
            ></b-form-file
          ></b-form-group>
          <b-form-group label-size="lg" label="Question Name">
            <ckeditor
...
export default {
  ...
  created() {
    this.getQuestionData(this.$route.params.chapter_id);
  },
  data() {
    return {
      isBusy: false,
  ...
  mounted() {
    if (this.questionData) {
      this.totalRows = this.questionData.length;
    }
    // Set the initial number of items
  },
  methods: {
    ...mapActions("question", ["getQuestionData"]),
    ...mapActions("question", ["storeQuestionRequest"]),
    ...mapActions("question", ["updateQuestionRequest"]),
    ...mapActions("question", ["destroyQuestionRequest"]),
    ...
    handleSubmit() {
      ...
      if (this.formData.isEdit == true) {
        ...
        this.isBusy = true;
        this.updateQuestionRequest(formData, config).then(() => {
          this.getCustomers();
        });
      } else {
        this.formData.id = this.$route.params.chapter_id;
        const config = {
          headers: { "content-type": "multipart/form-data" },
        };
        ...
        formData.append("question", this.formData.question);
        this.isBusy = true;
        this.storeQuestionRequest(formData, config).then(() => {
          this.getCustomers();
        });
      }
    ...
    MsgBoxDel(item, index, button) {
      this.$bvModal
        ...
        .then((value) => {
          if (value === true) {
            this.isBusy = true;
            this.destroyQuestionRequest(item.id).then(() => {
              this.getCustomers();
            });
          }
        })
